declare namespace jcy {
   ...
   * ```
   */
  export function KvPair(data: any): IKvPair;
   ...
   * @returns 拼接后的url地址
   */
  export function urlToGet(url: string, searchString: string | Record<string, any>): string;
   ...
   * @returns 字符串
   */
  export function trim(data: any, pos?: 1 | -1 | '*'): string;
   ...
   * @returns 树摇后的数据
   */
  export function shake<T extends TargetData>(data: T, options?: Options): BTDeepPartial<T>;
   ...
   * @returns 结果
   */
  export function repeatString(target: string | number, count: number): string;
   ...
   * @returns 改变后的索引
   */
  export function changeIndex(maxIndex: number, currentIndex: number, increase: number): number;
   ...
   * @returns 随机数
   */
  export function getRandom(min: number, max: number, isInt?: boolean): number;
   ...
   * @returns 转变后的新数据
   */
  export function main<T extends IKvPair, K extends IKvPair, C extends keyof T>(
    data: K[],
    childKeyTo: C,
   ...
   * ```
   */
  export function defineEventBus<T extends EventHander>(
    eventName: string
  ): {
    /** 追加订阅 */
    on: (hander: T) => void;
   ...
   */
  export function setCookie(key: string, value: string, expiredTime?: number): void;
   ...
   * @returns 键值对
   */
  export function parseQuery(str?: string): IKvPair;
